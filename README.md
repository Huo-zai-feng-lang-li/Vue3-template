...
<p align="center">
  <h3 align="center">🎉 VueHooks Plus</h3>
...
-  scripts 命令解释
-  执行 `pnpm run pnpm:` 会安装依赖并启动开发服务器
-  执行 `pnpm run dev` 会并行启动开发服务器和 Husky 安装（自动打开浏览器）
-  执行 `pnpm run CheckAll` 会并行运行 ESLint 检查和 Prettier 格式化检查
-  执行 `pnpm run commit` 会拉取代码、添加所有更改、使用 git-cz 提交、推送并自动构建发布页面(Github Pages)
-  执行 `pnpm run test` 会以测试模式运行项目
-  执行 `pnpm run prod` 会以生产模式运行项目
-  执行 `pnpm run build` 会运行代码检查后以生产模式构建项目
-  执行 `pnpm run build:dev` 会以开发模式构建项目
-  执行 `pnpm run build:test` 会以测试模式构建项目
